...
export class StarshipsPage {
  ...
  constructor(
    ...
    this.starships = this.getData();
    this.loading.dismiss();
  }

  getStarships() {
    this.swapi.getStarships().then(
      data => {
        this.metadata.count = data['count'];
        this.metadata.next = data['next'];
        this.starships = this.starships.concat(data['data']);
  ...
  loadMore(infiniteScroll) {
    ...
    if (this.starships.length >= this.metadata.count) {
  ...
  private getData() {
    ...
    return arr.map(elem => new Starship(elem));
  }
